...
class Lk21 : MainAPI() {
    override var mainUrl = "https://iwcsantodomingo.org"
    ...
    override val mainPage = mainPageOf(
        ...
        "$mainUrl/genre/animation/" to "⛩️ Animasi & Kartun (Animation)"
    )
    ...
    override suspend fun getMainPage(
        ...
        val items = doc.select("article.item, article.has-post-thumbnail, div.gmr-box-item, article.item-infinite, .post-item, div.item")
    ...
    private fun toSearchResult(element: Element): SearchResponse? {
        return runCatching {
            ...
            if (href.isBlank() || href == "$mainUrl/" || href.contains("/tag/") || href.contains("/category/") || href.contains("/country/") || href.contains("/year/") || href.contains("/page/") || href.contains("/genre/")) return null
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return doc.select("article.item, article.has-post-thumbnail, div.gmr-box-item, article.item-infinite, .post-item, div.item")
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val rawTitle = doc.selectFirst("h1.entry-title, .entry-title, .title-content, h1")?.text()?.trim() ?: "Lk21"
        val cleanTitle = rawTitle
            .removePrefix("Permalink ke ")
            .removePrefix("Permalink to ")
            .removePrefix("Nonton Film ")
