...
class AnichinProvider : MainAPI() {
    ...
    override val mainPage = mainPageOf(
        ...
        "seri/page/%d/?status=&type=Movie&order=update" to "Movie",
        "seri/page/%d/?order=popular" to "Rekomendasi"
    )
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val genres = document.select(".genres a, .genre a, a[href*='/genres/'], a[href*='/genre/']")
            .map { it.text().trim() }
    ...
    override suspend fun loadLinks(
        ...
        streamUrls.forEach { streamUrl ->
            try {
                ...
                if (streamUrl.contains("anichin.stream/")) {
                    ...
                    val idx = streamDoc.indexOf(".split('|')")
                    if (idx != -1) {
                        val kStart = streamDoc.lastIndexOf("'", idx)
                        val kStr = streamDoc.substring(kStart + 1, idx - 1)
                        val k = kStr.split("|")

                        val commaBeforeK = streamDoc.lastIndexOf(",", kStart)
                        val cStr = streamDoc.substring(commaBeforeK + 1, kStart - 1).trim()
                        val c = cStr.toInt()

                        val commaBeforeC = streamDoc.lastIndexOf(",", commaBeforeK)
                        val aStr = streamDoc.substring(commaBeforeC + 1, commaBeforeK).trim()
                        val a = aStr.toInt()

                        val pStart = streamDoc.indexOf("}(", commaBeforeC - 200) + 2
                        var pVal = streamDoc.substring(pStart + 1, commaBeforeC - 1)
                        ...
                        for (i in c - 1 downTo 0) {
                            if (i < k.size && k[i].isNotEmpty()) {
                                val rep = baseN(i, a)
                                pVal = pVal.replace(Regex("\\b$rep\\b"), k[i])
                            }
                        }

                        val m3u8Regex = Regex("""/hls/[a-zA-Z0-9_]+\.m3u8""")
                        m3u8Regex.find(pVal)?.value?.let { path ->
                            val fullM3u8 = "https://anichin.stream$path"
                            callback.invoke(
                                newExtractorLink(
                                    source = "Anichin Stream",
                                    name = "Premium (anichin.stream)",
                                    url = fullM3u8,
                                    type = ExtractorLinkType.M3U8
                                ) {
                                    referer = "https://anichin.stream/"
