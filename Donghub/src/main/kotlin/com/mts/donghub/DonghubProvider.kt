...
class DonghubProvider : MainAPI() {
    ...
    private suspend fun scrapeList(pageUrl: String): List<SearchResponse> {
        ...
        return doc.select(".listupd .bsx, .listupd .bs, .bsx, .bs, article.bs, .item, article, .swiper-slide, .animpost, article.animpost, .animepost, article.animepost, article.item, .film-poster, .item-anime, .epbox, .out-thumb, .milist, .post-item, .hentry").mapNotNull {
