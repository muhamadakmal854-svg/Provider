...
class TUBITV : MainAPI() {
    override var mainUrl = "https://tubitv.com"
    override var name = "TUBITV"
    ...
    private fun Media.toSearchResponse(): SearchResponse? {
        ...
        val dataPayload = TUBITVData(id = id, type = mediaTypeStr).toJson()
    ...
    override suspend fun load(url: String): LoadResponse? {
        val data: TUBITVData = try {
            parseJson<TUBITVData>(url)
        } catch (_: Exception) {
                ...
                try {
                    ...
                    if (cleanId.startsWith("tt")) {
                        ...
                        if (numId != null) {
                            ...
                            TUBITVData(numId, if (isTv) "tv" else "movie")
                        } else null
                    }
                } catch (_: Exception) {
                    null
    ...
    private suspend fun fetchSubtitles(
        ...
        try {
            ...
            val res = app.get(stremioUrl, timeout = 10L).parsedSafe<TUBITVSubResponse>()
    ...
    override suspend fun loadLinks(
        ...
        listOf(
            ...
            suspend {
                invokeTUBITVNative(tmdbId, season, episode, subtitleCallback, callback)
            }
        ).amap { call ->
            try {
                call.invoke()
    ...
    private suspend fun invokeMoviesAPI(
        ...
        res.sources?.forEach { src ->
            ...
            val serverName = "TUBITV - Server 1 (MoviesAPI)"
    ...
    private suspend fun invokeVidCore(
        ...
        res.streams?.forEach { stream ->
            ...
            val serverName = "TUBITV - Server 2 (VidCore [$label])"
    ...
    private suspend fun invokeVidrock(
        ...
        sources.forEach { source ->
            ...
            val serverName = "TUBITV - Server 3 (Vidrock [$sourceName])"
    ...
    private suspend fun invokeVidlink(
        ...
        try {
            loadExtractor(url, "https://tubitv.com/", subtitleCallback, callback)
        } catch (_: Throwable) {
        }
    }
    ...
    private suspend fun invokeTUBITVNative(
        tmdbId: Int,
        season: Int?,
        episode: Int?,
        subtitleCallback: (SubtitleFile) -> Unit,
        ...
        urls.forEach { embedUrl ->
            try {
                loadExtractor(embedUrl, "https://tubitv.com/", subtitleCallback, callback)
            } catch (_: Throwable) {
            }
        }
    }

    data class TUBITVData(
        val id: Int? = null,
        val type: String? = null
    )

    data class TUBITVLinkData(
        val id: Int? = null,
    ...
    data class TUBITVSubResponse(
        @JsonProperty("subtitles") val subtitles: List<TUBITVSubItem>? = null
    )

    data class TUBITVSubItem(
        @JsonProperty("url") val url: String? = null,
