...
class Stremio : MainAPI() {
    override var mainUrl = "https://web.strem.io"
    ...
    private fun Media.toSearchResponse(): SearchResponse? {
        ...
        val dataPayload = StremioData(id = id, type = mediaTypeStr).toJson()
    ...
    override suspend fun load(url: String): LoadResponse? {
        val data: StremioData = try {
            parseJson<StremioData>(url)
        } catch (_: Exception) {
            if (url.startsWith("http") || url.startsWith("tt")) {
                try {
                    ...
                    if (cleanId.startsWith("tt")) {
                        ...
                        if (movie?.id != null) {
                            StremioData(movie.id, "movie")
                        } else if (tv?.id != null) {
                            StremioData(tv.id, "tv")
                        } else null
                    } else {
                        val numId = cleanId.toIntOrNull()
                        if (numId != null) {
                            val isTv = url.contains("/tv") || url.contains("/series")
                            StremioData(numId, if (isTv) "tv" else "movie")
                        } else null
                    }
                } catch (_: Exception) {
                    null
    ...
    override suspend fun loadLinks(
        ...
        val linkData = try {
            parseJson<StremioLinkData>(data)
        } catch (_: Throwable) {
            null
        }
        ...
        listOf(
            ...
            suspend {
                invokeStremioNative(tmdbId, season, episode, subtitleCallback, callback)
            }
        ).amap { call ->
            try {
                call.invoke()
    ...
    private suspend fun invokeMoviesAPI(
        ...
        res.sources?.forEach { src ->
            ...
            val serverName = "Stremio - Server 1 (MoviesAPI)"
    ...
    private suspend fun invokeVidCore(
        ...
        res.streams?.forEach { stream ->
            ...
            val serverName = "Stremio - Server 2 (VidCore [$label])"
    ...
    private suspend fun invokeVidrock(
        ...
        sources.forEach { source ->
            ...
            val serverName = "Stremio - Server 3 (Vidrock [$sourceName])"
    ...
    private suspend fun invokeVidlink(
        ...
        try {
            loadExtractor(url, "https://web.strem.io/", subtitleCallback, callback)
        } catch (_: Throwable) {
        }
    }
    ...
    private suspend fun invokeStremioNative(
        tmdbId: Int,
        season: Int?,
        episode: Int?,
        subtitleCallback: (SubtitleFile) -> Unit,
        ...
        urls.forEach { embedUrl ->
            try {
                loadExtractor(embedUrl, "https://web.strem.io/", subtitleCallback, callback)
            } catch (_: Throwable) {
            }
        }
    }

    data class StremioData(
        val id: Int? = null,
        val type: String? = null
    )

    data class StremioLinkData(
        val id: Int? = null,
