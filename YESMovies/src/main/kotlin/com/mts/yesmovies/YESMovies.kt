...
class YESMovies : MainAPI() {
    override var mainUrl = "https://ww2.yesmovies.ag"
    ...
    override suspend fun getMainPage(
        ...
        val res = try {
            ...
            null
        } ?: throw ErrorLoadingException("Gagal memuatkan data dari pelayan YESMovies")
    ...
    private fun Media.toSearchResponse(): SearchResponse? {
        ...
        val dataPayload = YESMoviesData(id = id, type = mediaTypeStr).toJson()
    ...
    override suspend fun load(url: String): LoadResponse? {
        val data: YESMoviesData = try {
            parseJson<YESMoviesData>(url)
        } catch (_: Exception) {
            if (url.startsWith("http") || url.startsWith("tt")) {
                try {
                    ...
                    if (cleanId.startsWith("tt")) {
                        ...
                        if (movie?.id != null) {
                            YESMoviesData(movie.id, "movie")
                        } else if (tv?.id != null) {
                            YESMoviesData(tv.id, "tv")
                        } else null
                    } else {
                        val numId = cleanId.toIntOrNull()
                        if (numId != null) {
                            val isTv = url.contains("/tv") || url.contains("/series")
                            YESMoviesData(numId, if (isTv) "tv" else "movie")
                        } else null
                    }
                } catch (_: Exception) {
                    null
    ...
    private suspend fun fetchSubtitles(
        ...
        try {
            ...
            val res = app.get(stremioUrl, timeout = 10L).parsedSafe<YESMoviesSubResponse>()
    ...
    override suspend fun loadLinks(
        ...
        val linkData = try {
            parseJson<YESMoviesLinkData>(data)
        } catch (_: Throwable) {
            null
        }
        ...
        listOf(
            ...
            suspend {
                invokeYESMoviesNative(tmdbId, season, episode, subtitleCallback, callback)
            }
        ).amap { call ->
            try {
                call.invoke()
    ...
    private suspend fun invokeMoviesAPI(
        ...
        res.sources?.forEach { src ->
            ...
            val serverName = "YESMovies - Server 1 (MoviesAPI)"
    ...
    private suspend fun invokeVidCore(
        ...
        res.streams?.forEach { stream ->
            ...
            val serverName = "YESMovies - Server 2 (VidCore [$label])"
    ...
    private suspend fun invokeVidrock(
        ...
        sources.forEach { source ->
            ...
            val serverName = "YESMovies - Server 3 (Vidrock [$sourceName])"
    ...
    private suspend fun invokeVidlink(
        ...
        try {
            loadExtractor(url, "https://ww2.yesmovies.ag/", subtitleCallback, callback)
        } catch (_: Throwable) {
        }
    }
    ...
    private suspend fun invokeYESMoviesNative(
        tmdbId: Int,
        season: Int?,
        episode: Int?,
        subtitleCallback: (SubtitleFile) -> Unit,
        ...
        urls.forEach { embedUrl ->
            try {
                loadExtractor(embedUrl, "https://ww2.yesmovies.ag/", subtitleCallback, callback)
            } catch (_: Throwable) {
            }
        }
    }

    data class YESMoviesData(
        val id: Int? = null,
        val type: String? = null
    )

    data class YESMoviesLinkData(
        val id: Int? = null,
    ...
    data class YESMoviesSubResponse(
        @JsonProperty("subtitles") val subtitles: List<YESMoviesSubItem>? = null
    )

    data class YESMoviesSubItem(
        @JsonProperty("url") val url: String? = null,
