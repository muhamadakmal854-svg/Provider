...
class F2Movies : MainAPI() {
    override var mainUrl = "https://f2movies.sx"
    override var name = "F2Movies"
    ...
    override suspend fun getMainPage(
        ...
        val res = app.get(url).parsedSafe<Results>()
            ?: throw ErrorLoadingException("Gagal memuatkan data dari pelayan F2Movies")
    ...
    private fun Media.toSearchResponse(): SearchResponse? {
        ...
        val dataPayload = F2MoviesData(id = id, type = mediaTypeStr).toJson()
    ...
    override suspend fun load(url: String): LoadResponse? {
        val data: F2MoviesData = try {
            parseJson<F2MoviesData>(url)
        } catch (_: Exception) {
            if (url.contains("f2movies.sx/watch/")) {
                parseF2MoviesUrl(url)
            } else if (url.contains("/movie/")) {
                val id = url.substringAfterLast("/").substringAfterLast("-").toIntOrNull()
                F2MoviesData(id, "movie")
        ...
        val title = res.title ?: res.name ?: res.originalTitle ?: res.originalName ?: "F2Movies"
    ...
    private suspend fun parseF2MoviesUrl(url: String): F2MoviesData? {
        return try {
            val doc = app.get(
                url,
                headers = mapOf(
            ...
            val tmdbId = Regex("""(?:embed/(?:movie|tv)/|embed\?id=|movie/|tv/)(\d+)""").find(rawHtml)?.groupValues?.get(1)?.toIntOrNull()
                ?: Regex("""window\.__OPT=\[.*?(?:movie|tv)/(\d+)""").find(rawHtml)?.groupValues?.get(1)?.toIntOrNull()

            val isTv = url.contains("/watch/tv-") || rawHtml.contains("/tv/")
            F2MoviesData(
                id = tmdbId,
    ...
    override suspend fun loadLinks(
        ...
        val linkData = try {
            parseJson<F2MoviesLinkData>(data)
        } catch (_: Throwable) {
            null
        }
        ...
        listOf(
            ...
            // Pelayan 2: VidCore / Rigel (F2Movies Server 2 - 1080p FHD HLS)
            suspend {
    ...
    private suspend fun invokeMoviesAPI(
        ...
        res.sources?.forEach { src ->
            ...
            val serverName = "F2Movies - Server 1 (MoviesAPI)"
    ...
    private suspend fun invokeVidCore(
        ...
        res.streams?.forEach { stream ->
            ...
            val serverName = "F2Movies - Server 2 (VidCore [$label])"
    ...
    private suspend fun invokeVidrock(
        ...
        sources.forEach { source ->
            ...
            val serverName = "F2Movies - Server 3 (Vidrock [$sourceName])"
    ...
    data class F2MoviesData(
        val id: Int? = null,
        val type: String? = null
    )

    data class F2MoviesLinkData(
        val id: Int? = null,
