...
class GudangFilm : MainAPI() {
    override var mainUrl = "http://159.223.65.15"
    ...
    override val mainPage = mainPageOf(
        "$mainUrl/year/2026/" to "✨ Pilihan Utama (Spotlight / Trending 2026)",
        "$mainUrl/" to "🔥 Film Terbaru (Update Harian)",
        "$mainUrl/tv/" to "📺 TV Series & Drama Terkini",
        "$mainUrl/country/korea/" to "🎬 Drama & Film Korea (K-Drama)",
        ...
        "$mainUrl/genre/thriller/" to "🔍 Thriller & Jenayah (Crime & Thriller)",
        "$mainUrl/genre/adventure/" to "🗺️ Pengembaraan (Adventure)"
    )
    ...
    override suspend fun getMainPage(
        ...
        val items = doc.select("article, div.gmr-box-item, div.item, .item-infinite, .post-item, div.item-movie")
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return doc.select("article, div.gmr-box-item, div.item, .item-infinite, .post-item, div.item-movie")
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val rawTitle = doc.selectFirst("h1.entry-title, .entry-title, .title-content, h1")?.text()?.trim() ?: "GudangFilm"
        val cleanTitle = rawTitle
            .removePrefix("Permalink to ")
            .removePrefix("Nonton Film ")
            .removePrefix("Nonton ")
        ...
        val poster = getPosterUrl(doc.selectFirst("figure img, .gmr-poster-img img, .poster img, .cover img, img[src*='poster'], img[src*='backdrop']"))

        val plot = doc.selectFirst(".entry-content p, .gmr-moviedata-synopsis p, .synopsis p, .description p")?.text()?.trim()
        ...
        doc.select(".gmr-moviedata, .list-inline li, .entry-content-meta span, .metadata div, .details div, .info div").forEach { row ->
            ...
            when {
                ...
                text.startsWith("Rilis:", ignoreCase = true) || text.startsWith("Release:", ignoreCase = true) || text.startsWith("Posted on:", ignoreCase = true) || text.startsWith("Diposting pada:", ignoreCase = true) -> {
        ...
        val episodeElements = doc.select(".gmr-listseries a, a[href*='/eps/'], a[href*='/episode/']")
            .filter {
                val h = it.attr("href").trim()
                h.isNotBlank() && h != url && !h.endsWith("/tv/") && !h.endsWith("/tv")
            }
        ...
        if (isTvSeries && episodeElements.isNotEmpty()) {
            ...
            episodeElements.forEachIndexed { index, el ->
                ...
                val seasonMatch = Regex("""(?i)(?:Season|S)\s*(\d+)""").find("$epTooltip $epText $epHref")
                val seasonNum = seasonMatch?.groupValues?.get(1)?.toIntOrNull() ?: 1

                val epMatch = Regex("""(?i)(?:Episode|Eps|Ep|EP|E)\s*(\d+)""").find("$epTooltip $epText $epHref")
                val episodeNum = epMatch?.groupValues?.get(1)?.toIntOrNull() ?: (index + 1)

                val epName = if (epTooltip.isNotBlank()) {
                    epTooltip.removePrefix("Permalink to ")
    ...
    override suspend fun loadLinks(
        ...
        for (targetUrl in urlsToVisit) {
            ...
            // 2. Ambil server buttons & tab data
            doc.select("[data-url], [data-post], [data-nume], a.gmr-download-button, a[href*='download'], a[href*='drive']").forEach { btn ->
                val dataUrl = btn.attr("data-url").ifBlank { btn.attr("href") }.trim()
                if (dataUrl.isNotBlank() && !dataUrl.startsWith("#") && !dataUrl.startsWith("javascript:") && !dataUrl.contains("/tag/")) {
                    candidates.add(fixUrl(dataUrl))
                }
            }
        }

        // 3. Resolusi setiap calon server video
    ...
    private suspend fun resolveCandidate(
        ...
        // 1. VidHide / Morencius / Callistanise
