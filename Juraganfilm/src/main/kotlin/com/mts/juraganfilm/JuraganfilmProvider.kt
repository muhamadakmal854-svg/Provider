...
class JuraganfilmProvider : MainAPI() {
    ...
    override var name           = "JURAGANFILM : Situs Nonton Film Sub Indo Streaming Movie Online"
    ...
    private fun Element.parseToResponse(): SearchResponse? {
        ...
        val typeLabel = this.selectFirst(
    ...
    private suspend fun scrapeList(pageUrl: String): List<SearchResponse> {
        ...
        var items = if (doc != null) {
            doc.select("article.item, .item, article, .card, div.card, .movie-item, .post-item, div.module-item, div.ml-item, .box-item, .post, .entry, .film-poster-ahref").mapNotNull { it.parseToResponse() }
