...
class AnimeCube : MainAPI() {
    ...
    override val mainPage = mainPageOf(
        ...
        "$tmdbAPI/discover/movie?api_key=$apiKey&with_watch_providers=538&watch_region=US" to "⛩️ AnimeCube: Donghua & Animasi Terhangat",
        "$tmdbAPI/discover/tv?api_key=$apiKey&with_watch_providers=538&watch_region=US" to "🗡️ AnimeCube: Aksi & Xianxia / Fantasi Pilihan",
    ...
    private fun Media.toSearchResponse(): SearchResponse? {
        ...
        val dataPayload = AnimeCubeData(id = id, type = mediaTypeStr).toJson()
    ...
    override suspend fun load(url: String): LoadResponse? {
        val data: AnimeCubeData = try {
            parseJson<AnimeCubeData>(url)
        } catch (_: Exception) {
            if (url.startsWith("http") || url.startsWith("tt")) {
                try {
                    ...
                    if (cleanId.startsWith("tt")) {
                        ...
                        if (movie?.id != null) {
                            AnimeCubeData(movie.id, "movie")
                        } else if (tv?.id != null) {
                            AnimeCubeData(tv.id, "tv")
                        } else null
                    } else {
                        val numId = cleanId.toIntOrNull()
                        if (numId != null) {
                            val isTv = url.contains("/tv") || url.contains("/series")
                            AnimeCubeData(numId, if (isTv) "tv" else "movie")
                        } else null
                    }
                } catch (_: Exception) {
                    null
    ...
    override suspend fun loadLinks(
        ...
        val linkData = try {
            parseJson<AnimeCubeLinkData>(data)
        } catch (_: Throwable) {
            null
        }
        ...
        listOf(
            ...
            suspend {
                invokeAnimeCubeNative(tmdbId, season, episode, subtitleCallback, callback)
            }
        ).amap { call ->
            try {
                call.invoke()
    ...
    private suspend fun invokeVidlink(
        ...
        try {
            loadExtractor(url, "https://animecube.live/", subtitleCallback, callback)
        } catch (_: Throwable) {
        }
    }
