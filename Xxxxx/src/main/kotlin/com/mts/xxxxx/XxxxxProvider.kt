...
class XxxxxProvider : MainAPI() {

    override var mainUrl        = "https://xhaccess.com"
    override var name           = "Free Porn Videos & XXX Movies: Sex Videos Tube"
    ...
    private suspend fun scrapeList(pageUrl: String): List<SearchResponse> {
        ...
        return doc.select(".container-c9dbd, div.container-c9dbd, .thumb-list__item, div.thumb-list__item, .video-thumb, div.video-thumb, .video-thumb--type-video, div.video-thumb--type-video, .width-wrap, .card, div.card, article.item, .item, .movie-item, .post-item, div.module-item, div.ml-item, .box-item, article, .post, .entry, .film-poster-ahref").mapNotNull {
