...
class PopcornFlix : MainAPI() {
    override var mainUrl = "https://popcornflix.com"
    override var name = "PopcornFlix"
    ...
    override val mainPage = mainPageOf(
        ...
        "$tmdbAPI/discover/movie?api_key=$apiKey&with_watch_providers=538&watch_region=US" to "🍿 PopcornFlix: Filem Pilihan Percuma Terhangat",
        "$tmdbAPI/discover/tv?api_key=$apiKey&with_watch_providers=538&watch_region=US" to "📺 PopcornFlix: Siri TV Percuma Pilihan",
    ...
    private fun Media.toSearchResponse(): SearchResponse? {
        ...
        val dataPayload = PopcornFlixData(id = id, type = mediaTypeStr).toJson()
    ...
    override suspend fun load(url: String): LoadResponse? {
        val data: PopcornFlixData = try {
            parseJson<PopcornFlixData>(url)
        } catch (_: Exception) {
            if (url.startsWith("http") || url.startsWith("tt")) {
                try {
                    ...
                    if (cleanId.startsWith("tt")) {
                        ...
                        if (movie?.id != null) {
                            PopcornFlixData(movie.id, "movie")
                        } else if (tv?.id != null) {
                            PopcornFlixData(tv.id, "tv")
                        } else null
                    } else {
                        val numId = cleanId.toIntOrNull()
                        if (numId != null) {
                            val isTv = url.contains("/tv") || url.contains("/series")
                            PopcornFlixData(numId, if (isTv) "tv" else "movie")
                        } else null
                    }
                } catch (_: Exception) {
                    null
        ...
        val title = res.title ?: res.name ?: res.originalTitle ?: res.originalName ?: "PopcornFlix"
    ...
    private suspend fun fetchSubtitles(
        ...
        try {
            ...
            val res = app.get(stremioUrl, timeout = 10L).parsedSafe<PopcornFlixSubResponse>()
    ...
    override suspend fun loadLinks(
        ...
        val linkData = try {
            parseJson<PopcornFlixLinkData>(data)
        } catch (_: Throwable) {
            null
        }
        ...
        listOf(
            ...
            suspend {
                invokePopcornFlixNative(tmdbId, season, episode, subtitleCallback, callback)
            }
        ).amap { call ->
            try {
                call.invoke()
    ...
    private suspend fun invokeMoviesAPI(
        ...
        res.sources?.forEach { src ->
            ...
            val serverName = "PopcornFlix - Server 1 (MoviesAPI)"
    ...
    private suspend fun invokeVidCore(
        ...
        res.streams?.forEach { stream ->
            ...
            val serverName = "PopcornFlix - Server 2 (VidCore [$label])"
    ...
    private suspend fun invokeVidrock(
        ...
        sources.forEach { source ->
            ...
            val serverName = "PopcornFlix - Server 3 (Vidrock [$sourceName])"
    ...
    private suspend fun invokePopcornFlixNative(
        tmdbId: Int,
        season: Int?,
        episode: Int?,
        subtitleCallback: (SubtitleFile) -> Unit,
    ...
    data class PopcornFlixData(
        val id: Int? = null,
        val type: String? = null
    )

    data class PopcornFlixLinkData(
        val id: Int? = null,
    ...
    data class PopcornFlixSubResponse(
        @JsonProperty("subtitles") val subtitles: List<PopcornFlixSubItem>? = null
    )

    data class PopcornFlixSubItem(
        @JsonProperty("url") val url: String? = null,
