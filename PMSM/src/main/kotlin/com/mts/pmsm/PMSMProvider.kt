...
class PMSMProvider : BaseFixProvider() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val isHomepage = pageUrl == mainUrl || pageUrl == "$mainUrl/"
        val items = if (isHomepage && page == 1) {
            val isPmsm = mainUrl.contains("pencurimoviesubmalay")
            if (isPmsm) {
                when (request.name) {
                    "Featured" -> document.select("div.featured-tab div.module-item, div.featured-tab div.display-item")
                    "Movies" -> document.select("div.home-module.movies div.module-item, div.home-module.movies div.display-item")
                    "TV Shows" -> document.select("div.home-module.tvshows div.module-item, div.home-module.tvshows div.display-item")
                    "Episodes" -> document.select("div.home-module.episodes div.module-item, div.home-module.episodes div.display-item")
                    else -> document.select("div.module-item, div.ml-item, div.display-item")
                }
            } else {
                val tabPanes = document.select("div.tab-pane")
                if (tabPanes.size >= 3) {
                    when (request.name) {
                        "Latest Movies" -> tabPanes[0].select("div.ml-item, div.module-item, div.display-item")
                        "Latest TV Series" -> tabPanes[1].select("div.ml-item, div.module-item, div.display-item")
                        "Latest Episodes" -> tabPanes[2].select("div.ml-item, div.module-item, div.display-item")
                        else -> document.select("div.module-item, div.ml-item, div.display-item")
                    }
                } else {
                    document.select("div.module-item, div.ml-item, div.display-item")
                }
            }
        } else {
            document.select("div.module-item, div.ml-item, div.display-item")
