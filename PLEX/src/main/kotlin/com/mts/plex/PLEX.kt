...
class PLEX : MainAPI() {
    ...
    private fun Media.toSearchResponse(): SearchResponse? {
        ...
        val dataPayload = PLEXData(id = id, type = mediaTypeStr).toJson()
    ...
    override suspend fun load(url: String): LoadResponse? {
        val data: PLEXData = try {
            parseJson<PLEXData>(url)
        } catch (_: Exception) {
                ...
                try {
                    ...
                    if (cleanId.startsWith("tt")) {
                        ...
                        if (numId != null) {
                            ...
                            PLEXData(numId, if (isTv) "tv" else "movie")
                        } else null
                    }
                } catch (_: Exception) {
                    null
        ...
        val title = res.title ?: res.name ?: res.originalTitle ?: res.originalName ?: "PLEX"
    ...
    private suspend fun fetchSubtitles(
        ...
        try {
            ...
            val res = app.get(stremioUrl, timeout = 10L).parsedSafe<PLEXSubResponse>()
    ...
    override suspend fun loadLinks(
        ...
        listOf(
            ...
            suspend {
                invokePLEXNative(tmdbId, season, episode, subtitleCallback, callback)
            }
        ).amap { call ->
            try {
                call.invoke()
    ...
    private suspend fun invokeMoviesAPI(
        ...
        res.sources?.forEach { src ->
            ...
            val serverName = "PLEX - Server 1 (MoviesAPI)"
    ...
    private suspend fun invokeVidCore(
        ...
        res.streams?.forEach { stream ->
            ...
            val serverName = "PLEX - Server 2 (VidCore [$label])"
    ...
    private suspend fun invokeVidrock(
        ...
        sources.forEach { source ->
            ...
            val serverName = "PLEX - Server 3 (Vidrock [$sourceName])"
    ...
    private suspend fun invokeVidlink(
        ...
        try {
            loadExtractor(url, "https://watch.plex.tv/", subtitleCallback, callback)
        } catch (_: Throwable) {
        }
    }
    ...
    private suspend fun invokePLEXNative(
        tmdbId: Int,
        season: Int?,
        episode: Int?,
        subtitleCallback: (SubtitleFile) -> Unit,
        ...
        urls.forEach { embedUrl ->
            try {
                loadExtractor(embedUrl, "https://watch.plex.tv/", subtitleCallback, callback)
            } catch (_: Throwable) {
            }
        }
    }
    ...
    data class PLEXSubResponse(
        @JsonProperty("subtitles") val subtitles: List<PLEXSubItem>? = null
    )

    data class PLEXSubItem(
        @JsonProperty("url") val url: String? = null,
