...
class HDToday : MainAPI() {
    override var mainUrl = "https://hhdtoday.tv"
    ...
    override suspend fun getMainPage(
        ...
        val res = try {
            ...
            null
        } ?: throw ErrorLoadingException("Gagal memuatkan data dari pelayan HDToday")
    ...
    private fun Media.toSearchResponse(): SearchResponse? {
        ...
        val dataPayload = HDTodayData(id = id, type = mediaTypeStr).toJson()
    ...
    override suspend fun load(url: String): LoadResponse? {
        val data: HDTodayData = try {
            parseJson<HDTodayData>(url)
        } catch (_: Exception) {
            if (url.startsWith("http") || url.startsWith("tt")) {
                try {
                    ...
                    if (cleanId.startsWith("tt")) {
                        ...
                        if (movie?.id != null) {
                            HDTodayData(movie.id, "movie")
                        } else if (tv?.id != null) {
                            HDTodayData(tv.id, "tv")
                        } else null
                    } else {
                        val numId = cleanId.toIntOrNull()
                        if (numId != null) {
                            val isTv = url.contains("/tv") || url.contains("/series")
                            HDTodayData(numId, if (isTv) "tv" else "movie")
                        } else null
                    }
                } catch (_: Exception) {
                    null
    ...
    private suspend fun fetchSubtitles(
        ...
        try {
            ...
            val res = app.get(stremioUrl, timeout = 10L).parsedSafe<HDTodaySubResponse>()
    ...
    override suspend fun loadLinks(
        ...
        val linkData = try {
            parseJson<HDTodayLinkData>(data)
        } catch (_: Throwable) {
            null
        }
        ...
        listOf(
            ...
            suspend {
                invokeHDTodayNative(tmdbId, season, episode, subtitleCallback, callback)
            }
        ).amap { call ->
            try {
                call.invoke()
    ...
    private suspend fun invokeMoviesAPI(
        ...
        res.sources?.forEach { src ->
            ...
            val serverName = "HDToday - Server 1 (MoviesAPI)"
    ...
    private suspend fun invokeVidCore(
        ...
        res.streams?.forEach { stream ->
            ...
            val serverName = "HDToday - Server 2 (VidCore [$label])"
    ...
    private suspend fun invokeVidrock(
        ...
        sources.forEach { source ->
            ...
            val serverName = "HDToday - Server 3 (Vidrock [$sourceName])"
    ...
    private suspend fun invokeVidlink(
        ...
        try {
            loadExtractor(url, "https://hhdtoday.tv/", subtitleCallback, callback)
        } catch (_: Throwable) {
        }
    }
    ...
    private suspend fun invokeHDTodayNative(
        tmdbId: Int,
        season: Int?,
        episode: Int?,
        subtitleCallback: (SubtitleFile) -> Unit,
        ...
        urls.forEach { embedUrl ->
            try {
                loadExtractor(embedUrl, "https://hhdtoday.tv/", subtitleCallback, callback)
            } catch (_: Throwable) {
            }
        }
    }

    data class HDTodayData(
        val id: Int? = null,
        val type: String? = null
    )

    data class HDTodayLinkData(
        val id: Int? = null,
    ...
    data class HDTodaySubResponse(
        @JsonProperty("subtitles") val subtitles: List<HDTodaySubItem>? = null
    )

    data class HDTodaySubItem(
        @JsonProperty("url") val url: String? = null,
