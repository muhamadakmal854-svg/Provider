...
class WeTV : MainAPI() {
    override var mainUrl = "https://wetv.vip"
    override var name = "WeTV"
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val items = doc.select("a[href*=/play/]").mapNotNull {
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        val searchUrl = "$mainUrl/search?q=${query.replace(" ", "%20")}"
        val html = app.get(searchUrl).text
        val doc = Jsoup.parse(html)
        return doc.select("a[href*=/play/]").mapNotNull {
            val href = it.attr("href")
            val title = it.text().trim().ifBlank { it.select("img").attr("alt") }
            val img = it.select("img").attr("src")
            if (href.isBlank() || title.isBlank()) return@mapNotNull null
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        doc.select("a[href*=/play/]").forEachIndexed { idx, it ->
    ...
    override suspend fun loadLinks(
        ...
        // WeTV uses DRM/encrypted streams, passing fallback link if extracted from raw page
        return false
    }
}
