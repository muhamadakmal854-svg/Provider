...
class XnxxProvider : MainAPI() {
    ...
    private suspend fun scrapeList(pageUrl: String): List<SearchResponse> {
        ...
        val primaryCards = doc.select("article, .item, .post, .video-item, .thumb, .video-block, .clip, .movie-item")
        val cards = if (primaryCards.isNotEmpty()) primaryCards else {
            doc.select("a[href]").mapNotNull { a ->
                ...
                val hasTitle = a.attr("title").isNotBlank() || candidate.selectFirst("h1, h2, h3, h4, .title, .name, .video-title") != null || a.text().isNotBlank()
        ...
        return cards.mapNotNull {
            ...
            val img = it.selectFirst("img, [data-src]")
            val title = it.selectFirst("h2, h3, h4, .title, .name, .video-title")?.text()?.trim()
                ?: a.attr("title").trim().ifEmpty { img?.attr("alt")?.trim() ?: "" }.ifEmpty { img?.attr("title")?.trim() ?: "" }.ifEmpty { a.text().trim() }
            if (title.isBlank()) return@mapNotNull null
            val src = img?.posterUrl() ?: ""
            newMovieSearchResponse(title, href, TvType.NSFW) { posterUrl = src }
        }.distinctBy { it.url }
    }
