...
class Klikxxi : MainAPI() {
    override var mainUrl = "https://yearone-movie.com"
    ...
    override val mainPage = mainPageOf(
        ...
        "$mainUrl/category/movie/" to "🔥 Film Terbaru (Update Harian)",
        "$mainUrl/category/serial-tv/" to "📺 TV Series & Drama Terkini",
        "$mainUrl/category/box-office/" to "⭐ Box Office Pilihan (Top Hits)",
        "$mainUrl/category/anime/" to "⛩️ Anime & Animasi Terkini",
        "$mainUrl/category/donghua/" to "🐉 Donghua & Animasi China",
        ...
        "$mainUrl/category/romance/" to "💖 Percintaan & Romantis (Romance)"
    )
    ...
    override suspend fun getMainPage(
        ...
        val items = doc.select("article.item, article.has-post-thumbnail, div.gmr-box-item, article.item-infinite, .post-item")
    ...
    private fun toSearchResult(element: Element): SearchResponse? {
        return runCatching {
            ...
            if (href.isBlank() || href == "$mainUrl/" || href.contains("/tag/") || href.contains("/category/") || href.contains("/country/") || href.contains("/year/") || href.contains("/page/")) return null
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return doc.select("article.item, article.has-post-thumbnail, div.gmr-box-item, article.item-infinite, .post-item")
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val rawTitle = doc.selectFirst("h1.entry-title, .entry-title, .title-content, h1")?.text()?.trim() ?: "Klikxxi"
        val cleanTitle = rawTitle
            .removePrefix("Permalink ke ")
            .removePrefix("Permalink to ")
            .removePrefix("Nonton Film ")
        ...
        val poster = getPosterUrl(doc.selectFirst("figure img, .gmr-poster-img img, .poster img, .cover img, img.wp-post-image, img.attachment-large"))

        val plot = doc.selectFirst(".entry-content p, .description p, [itemprop='description'], .desc p")?.text()?.trim()
        ...
        doc.select(".gmr-moviedata, .list-inline li, .metadata div, .details div, .info div, .entry-content-meta span").forEach { row ->
        ...
        val seasonBlocks = doc.select("div.gmr-season-block, .season-list, .episodelst")
        val allEpisodes = mutableListOf<Episode>()

        if (seasonBlocks.isNotEmpty()) {
            seasonBlocks.forEach { block ->
                val seasonTitle = block.selectFirst("h3.season-title, .season-title")?.text()?.trim()
    ...
    override suspend fun loadLinks(
        ...
        // 1. Ambil direct iframes & embeds dari halaman utama
